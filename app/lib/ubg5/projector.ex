defmodule Ubg5.Projector do
  ...
  def set_verse(proj_id, bible, book_name, chapter_number, verse_number) do
    GenServer.cast(
      __MODULE__,
      {:set_verse, proj_id, bible, book_name, chapter_number, verse_number}
    )
  end
  ...
  def handle_call({:get_verse, proj_id}, _from, state) do
    result =
      case Map.fetch(state, proj_id) do
        :error -> nil
        {:ok, value} -> value
      end
  ...
  def handle_cast({:set_verse, proj_id, bible, book_name, chapter_number, verse_number}, state) do
    verse =
      Bibles.get_verses(bible, book_name, chapter_number, verse_number, verse_number)
      |> List.first()

    {:ok, structure} = Bibles.get_structure(bible)
